...
test("does not leak the phone number anywhere on the homepage", async ({
  page,
}) => {
  await page.goto("/");
...
test("a flagship case study shows repository link and limitations", async ({
  page,
}) => {
  await page.goto("/systems/code-review-arena");
  await expect(
    page.getByRole("heading", { name: "Code Review Arena" }),
  ).toBeVisible();
  await expect(page.getByText("Honest limitations")).toBeVisible();
  const repo = page
    .locator('a[href="https://github.com/harihkk/code-review-arena"]')
    .first();
  await expect(repo).toBeVisible();
});

test("research paper is labeled external and links to arXiv", async ({
  page,
}) => {
  await page.goto("/research/swe-bench");
  await expect(
    page.getByText("External paper · not authored by Hari"),
  ).toBeVisible();
