...
> Caveat: The audited clone of the live source captured here is empty (it contained no prose, titles, or assets, see `docs/content-conflicts.md` and the audit `gitMetadata`/`conflictsWithResume` notes). This means the _real_ live content lives in the deployed Pages site / a different branch, not in that clone. The recoverability guarantee rests on the live Pages repo being **unmodified**, plus the separately retained snapshot, not on that empty clone.
